...
- `/mute <user_id>` (или ответом на уведомление со ссылкой вида
  `tg://user?id=123`), `/unmute <user_id>` — для одного чата.
...
Уведомления о пропущенных/грубых сообщениях содержат кликабельную ссылку на
чат (`tg://user?id=...`), а не голый id — тапнешь и сразу открывается диалог.
